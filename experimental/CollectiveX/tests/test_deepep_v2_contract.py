...
class DeepEPV2ContractTests(unittest.TestCase):
    ...
    def test_runtime_pins_uccl_wheel_and_hybrid_source_tree(self) -> None:
        ...
        self.assertIn("scale-out NCCL network mode is not IB", reference)
        self.assertIn("scale-out RCCL must auto-select its HCA-pinned network", reference)
    ...
    def test_private_scheduler_diagnostic_is_value_free(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertIn("scheduler-diagnostic=resource-request", result.stderr)
