...
class PrecisionSchedulingTest(unittest.TestCase):
    ...
    def test_capability_registry_uses_exact_native_targets(self) -> None:
        ...
        self.assertEqual(
            len(targets) - len(capability.provisional_precision_targets()), 78
        )
        self.assertEqual(
            sum(item["disposition"] == "supported" for item in targets), 59
        )
        self.assertEqual(
            sum(item["disposition"] == "unsupported" for item in targets), 19
        )
        self.assertEqual(len(capability.provisional_precision_targets()), 16)
        keys = {
            (
                item["precision_profile"],
                item["backend"],
