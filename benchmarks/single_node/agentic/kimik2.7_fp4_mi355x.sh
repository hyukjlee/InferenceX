...
case "$OFFLOAD_MODE" in
    ...
    lmcache)
        ...
        # Server flags below track the MI355X Kimi-K2.7 LMCache runbook.
        # chunk-size 1024: fewer server round-trips than the 256 default; the MP
...
VLLM_CMD=(
    ...
    # without these the agent harness can't parse message.tool_calls. Harmless
    # for the throughput replay path.
...
# ---- Run benchmark or accuracy eval -----------------------------------------
# EVAL_ONLY=true (workflow eval-only input) runs the SWE-bench agentic accuracy
# eval against the live server instead of the throughput replay; run_eval
# dispatches to the swebench harness for agentic-coding scenarios.
